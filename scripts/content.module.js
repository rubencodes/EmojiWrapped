...
/* harmony import */ var _utilities_format_seconds__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../utilities/format-seconds */ "./src/utilities/format-seconds.ts");
		...
		<div className="app-loaded-state">
			...
			<div className="app-footer">
				<p
					...
					title="${new Date(startTime).toLocaleString()} to ${new Date(endTime).toLocaleString()}"
				>
					Elapsed time: ${(0,_utilities_format_seconds__WEBPACK_IMPORTED_MODULE_5__.formatSeconds)(elapsedSeconds)}
				</p>
				<div className="app-buttons">
...
function useCachedState(keyPath, defaultValue, onLoadFromCache) {
    ...
    (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
        (() => __awaiter(this, void 0, void 0, function* () {
            let cachedState = yield _utilities_Cache__WEBPACK_IMPORTED_MODULE_1__.Cache.get(keyPath);
            if (cachedState) {
                setState(cachedState);
            }
            // Migrate legacy stats to new cache key.
            if (typeof cachedState === "undefined" &&
                keyPath === _utilities_Cache__WEBPACK_IMPORTED_MODULE_1__.CacheKey.SearchResults) {
                cachedState = (yield _utilities_Cache__WEBPACK_IMPORTED_MODULE_1__.Cache.get(_utilities_Cache__WEBPACK_IMPORTED_MODULE_1__.CacheKey.Stats));
                if (cachedState) {
                    setState(cachedState);
                }
            }
    ...
    const setCachedState = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useCallback)((value) => {
        setState((currentValue) => {
            if (typeof value === "function") {
                ...
                _utilities_Cache__WEBPACK_IMPORTED_MODULE_1__.Cache.set(keyPath, newValue);
                return newValue;
            }
...
/* harmony import */ var _use_eta__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./use-eta */ "./src/hooks/use-eta.ts");
/* harmony import */ var _utilities_Cache__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../utilities/Cache */ "./src/utilities/Cache.ts");
/* harmony import */ var _utilities_API__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../utilities/API */ "./src/utilities/API.ts");
/* harmony import */ var _utilities_validate_stats_json__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../utilities/validate-stats-json */ "./src/utilities/validate-stats-json.ts");
/* harmony import */ var _types_app_state__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../types/app-state */ "./src/types/app-state.ts");
var __awaiter = (undefined && undefined.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
...
function useEmoji() {
    ...
    const [state, setState] = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useState)(_types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.Initial);
    // Cached data from the network.
    const [emojis, setEmojis] = (0,_use_cached_state__WEBPACK_IMPORTED_MODULE_1__.useCachedState)(_utilities_Cache__WEBPACK_IMPORTED_MODULE_3__.CacheKey.Emoji, []);
    const [searchResults, setSearchResults] = (0,_use_cached_state__WEBPACK_IMPORTED_MODULE_1__.useCachedState)(_utilities_Cache__WEBPACK_IMPORTED_MODULE_3__.CacheKey.SearchResults, [], (cachedData) => {
        setState(cachedData && cachedData.length > 0 ? _types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.Loaded : _types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.Empty);
    });
    // Compile stats from search results when emojis and search results are loaded.
    const [stats, setStats] = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useState)([]);
    (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
        if (emojis.length === 0)
            return;
        ...
        const stats = searchResults
            ...
            const originalEmojiIndex = stats.findIndex((e) => (emoji.alias_for && e.name === emoji.alias_for) ||
                e.name === emoji.name);
            const originalEmoji = originalEmojiIndex >= 0 ? stats[originalEmojiIndex] : null;
            // If the alias emoji is found, update the original emoji.
            if (originalEmoji) {
                const updated = Object.assign(Object.assign({}, originalEmoji), { createdAt: Math.min(originalEmoji.createdAt, ((_a = emoji.created) !== null && _a !== void 0 ? _a : 0) * 1000), items: [...originalEmoji.items, ...items], count: originalEmoji.count + count });
                return [
            ...
            if (emoji.alias_for) {
                const newEmoji = {
                    ...
                    url: emoji.url,
                    createdAt: ((_b = emoji.created) !== null && _b !== void 0 ? _b : 0) * 1000,
                    items,
                    count,
                };
            ...
            .sort((a, b) => b.count - a.count);
        setStats(stats);
        setState(_types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.Loaded);
    ...
    const [startTime, setStartTime] = (0,_use_cached_state__WEBPACK_IMPORTED_MODULE_1__.useCachedState)(_utilities_Cache__WEBPACK_IMPORTED_MODULE_3__.CacheKey.StartTime, 0);
    const [endTime, setEndTime] = (0,_use_cached_state__WEBPACK_IMPORTED_MODULE_1__.useCachedState)(_utilities_Cache__WEBPACK_IMPORTED_MODULE_3__.CacheKey.EndTime, 0);
    ...
    const loadStats = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useCallback)((forced) => __awaiter(this, void 0, void 0, function* () {
        ...
        setState(_types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.LoadingEmoji);
        // Fetch the emoji list, reading from the cache if possible.
        const updatedEmojis = !forced && emojis && emojis.length > 0
            ? emojis
            : yield _utilities_API__WEBPACK_IMPORTED_MODULE_4__.API.fetchEmojiList({ setPercentLoaded });
        if (forced || !emojis || !emojis.length)
            setEmojis(updatedEmojis);
        setPercentLoaded(1);
        // Fetch the emoji stats, reading from the cache if possible.
        const updatedSearchResults = !forced && searchResults && searchResults.length > 0
            ? searchResults
            : [];
        try {
            if (updatedSearchResults.length === 0) {
                setState(_types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.LoadingStats);
                setPercentLoaded(0);
                for (const emoji of updatedEmojis) {
                    const index = updatedEmojis.findIndex((e) => e === emoji);
                    const { count, items } = yield _utilities_API__WEBPACK_IMPORTED_MODULE_4__.API.fetchEmojiUsage({
                        year,
                        emoji: emoji.name,
                    });
            ...
            setState(_types_app_state__WEBPACK_IMPORTED_MODULE_6__.State.Error);
        }
        finally {
            setPercentLoaded(1);
    ...
    const importStats = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useCallback)((event) => {
        ...
        reader.onload = (e) => {
            ...
            const json = (0,_utilities_validate_stats_json__WEBPACK_IMPORTED_MODULE_5__.validateStatsJSON)(jsonData);
            if (!json)
                return;
            setYear(json.year);
    ...
    const eta = (0,_use_eta__WEBPACK_IMPORTED_MODULE_2__.useETA)({
        state,
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   useETA: () => (/* binding */ useETA)
/* harmony export */ });
/* harmony import */ var preact_hooks__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! preact/hooks */ "./node_modules/preact/hooks/dist/hooks.module.js");
/* harmony import */ var _utilities_format_seconds__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../utilities/format-seconds */ "./src/utilities/format-seconds.ts");
/* harmony import */ var _types_app_state__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../types/app-state */ "./src/types/app-state.ts");



function useETA({ state, emojiCount, startTime, percentLoaded, defaultRate = 1, }) {
    const [rate, setRate] = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useState)(defaultRate);
    const calculateETA = (0,preact_hooks__WEBPACK_IMPORTED_MODULE_0__.useCallback)((emojiRemaining) => {
        if (state !== _types_app_state__WEBPACK_IMPORTED_MODULE_2__.State.LoadingStats)
            return undefined;
        const eta = Math.round(emojiRemaining * rate);
        return (0,_utilities_format_seconds__WEBPACK_IMPORTED_MODULE_1__.formatSeconds)(eta);
    }, [rate, state]);
...
(function (CacheKey) {
    ...
    CacheKey["SearchResults"] = "searchResultsCache2";
...
/***/ "./src/utilities/format-seconds.ts":
/*!*****************************************!*\
  !*** ./src/utilities/format-seconds.ts ***!
  \*****************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
function validateStatsJSON(jsonData) {
    try {
        let { year, emojis, searchResults, stats, startTime, endTime } = JSON.parse(jsonData);
        ...
        if (!Array.isArray(searchResults)) {
            ...
            if (stats.some(({ name, items, count }) => typeof name !== "string" ||
                !Array.isArray(items) ||
        ...
        if (searchResults.some(({ name, items, count }) => typeof name !== "string" ||
            !Array.isArray(items) ||
            items.some(({ messages }) => !Array.isArray(messages)) ||
            typeof count !== "number")) {
